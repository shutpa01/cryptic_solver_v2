        ...
        for L in range(1, N - p + 1):
            ...
            for (ia, ib) in runs:
                ...
                for (oa, ob) in runs:
                    ...
                    # PHRASE-AWARE residue split (was per-word is_con + links, which
                    # stranded half of a multi-word indicator and rejected the parse).
                    ...
                    if sp is not None:
                        return {"p": p, "L": L, "inner": (ia, ib),
                                "outer": (oa, ob), "con": sp[0], "links": sp[1]}
    return None
